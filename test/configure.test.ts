...
describe('configure', () => {
  it('Should create decorator for urlcat with a configuration', () => {
    ...
    const urlcat = configure({ objectFormat: { format: 'RFC3986' } })

    const actual = urlcat('http://example.com/path/:p', { p: 'a b', q: 'b c' })
    expect(actual).toBe(expected)
  })

  it('Should create decorator for urlcat and override with custom configuration', () => {
    const expected = 'http://example.com/path/a%20b?q=b%20c'
  ...
  it('Should create decorator for urlcat with all configuration options', () => {
    const expected = 'http://example.com/path/a%20b?q=b%20c&c=foo&c=bar'
    const urlcat = configure({
      objectFormat: { format: 'RFC3986' },
      arrayFormat: 'repeat',
    })
  ...
  it('Should create decorator for urlcat with all configuration options and override with custom configuration', () => {
    const expected = 'http://example.com/path/a%20b?q=b%20c&c=foo&c=bar'
    const urlcat = configure({
      objectFormat: { format: 'RFC1738' },
      arrayFormat: 'repeat',
    })

    const actual = urlcat(
      'http://example.com/path/:p',
      { p: 'a b', q: 'b c', c: ['foo', 'bar'] },
      undefined,
      { arrayFormat: 'comma', objectFormat: { format: 'RFC3986' } },
    )
    expect(actual).toBe(expected)
  })
})
