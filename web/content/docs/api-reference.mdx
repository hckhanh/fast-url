...
const url = urlcat('http://api.example.com/users/:id', { 
  id: 42, 
  search: 'foo',
  active: true
});
...
```typescript
function urlcat(baseUrl: string, path: string): string
```
...
const url1 = urlcat('http://api.example.com/', '/users');
console.log(url1);
// → 'http://api.example.com/users'

// Works without slashes too
const url2 = urlcat('http://api.example.com', 'users');
console.log(url2);
// → 'http://api.example.com/users'
```
...
```typescript
import urlcat, { query } from 'fast-url';
...
class GitHubAPI {
  ...
  getUserRepos(username: string, options: {
    ...
    return urlcat(this.baseUrl, '/users/:username/repos', {
      username,
      ...options
    });
  }
  ...
  getCommit(owner: string, repo: string, sha: string) {
    return urlcat(this.baseUrl, '/repos/:owner/:repo/commits/:sha', {
      owner,
      repo,
      sha
    });
  ...
  // Search repositories
  searchRepos(q: string, options: {
    sort?: 'stars' | 'forks' | 'updated';
    order?: 'asc' | 'desc';
    per_page?: number;
  } = {}) {
    return urlcat(this.baseUrl, '/search/repositories', {
      q,
      ...options
    });
  }
...
console.log(github.getUserRepos('octocat', { 
  type: 'owner', 
  sort: 'updated',
  per_page: 50 
}));
...
console.log(github.searchRepos('fast-url language:typescript', { 
  sort: 'stars',
  order: 'desc'
}));
...
function route(
  ...
  return urlcat('https://myblog.com', routes[name], params);
}
...
<Cards>
  ...
  <Card title="Introduction" href="/docs/introduction">
    Understand why fast-url was created and its key features
  </Card>
</Cards>
