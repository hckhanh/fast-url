...
<Tabs items={['jsr', 'bun', 'npm', 'yarn', 'pnpm']}>
  <Tab value="jsr">```bash deno add jsr:@hckhanh/fast-url ```</Tab>
  <Tab value="bun">```bash bun add fast-url ```</Tab>
  <Tab value="npm">```bash npm install fast-url ```</Tab>
  <Tab value="yarn">```bash yarn add fast-url ```</Tab>
  <Tab value="pnpm">```bash pnpm add fast-url ```</Tab>
</Tabs>
...
<Tabs items={['ES Modules', 'CommonJS', 'TypeScript']}>
  ...
  <Tab value="TypeScript">
    ```typescript
    import { createUrl, query, subst, join, type ParamMap } from 'fast-url';
    ```
  </Tab>
</Tabs>
