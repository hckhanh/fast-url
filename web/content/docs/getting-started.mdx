...
<Tabs items={['ES Modules', 'CommonJS', 'TypeScript']}>
  ...
  <Tab value="CommonJS">
    ...
    const { query, subst, join } = require('fast-url');
    ```
  </Tab>
  <Tab value="TypeScript">
    ```typescript
    import urlcat, { query, subst, join, type ParamMap } from 'fast-url';
    ```
  </Tab>
</Tabs>
...
const userId = 123;
const url = urlcat('https://api.example.com', '/users/:id', { id: userId });
console.log(url);
...
const url = urlcat('https://api.example.com', '/users', { 
  limit: 10, 
  offset: 20,
  sort: 'name'
});
...
const url = urlcat('https://api.example.com', '/users/:id/posts', { 
  id: 123, 
  limit: 10,
  published: true
});
...
<Steps>
  ...
  <Step>
    ...
    Use urlcat to build the endpoint URL with query parameters:
    
    ```typescript
    async getPosts(options = {}) {
      const url = urlcat(this.baseUrl, '/posts', {
        page: options.page || 1,
  ...
  <Step>
    ...
    async getPost(postId: number) {
      const url = urlcat(this.baseUrl, '/posts/:id', { id: postId });
...
const url = urlcat('https://api.example.com', '/search', {
  q: 'javascript',
...
const url1 = urlcat('https://api.example.com', '/search', {
...
const url2 = urlcat('https://api.example.com', '/users/:name', {
  name: '田中太郎'
});
// → 'https://api.example.com/users/%E7%94%B0%E4%B8%AD%E5%A4%AA%E9%83%8E'

// Even emojis work correctly
const url3 = urlcat('https://api.example.com', '/posts', {
  mood: '😊🎉'
});
...
try {
  ...
  urlcat('https://api.example.com', '/users/:id', {});
} catch (error) {
...
try {
  ...
  urlcat('https://api.example.com', '/users/:id', { id: '' });
} catch (error) {
  console.error(error.message);
...
<Cards>
  ...
  <Card title="Introduction" href="/docs/introduction">
    Learn more about why fast-url was created and its key features
  </Card>
</Cards>
